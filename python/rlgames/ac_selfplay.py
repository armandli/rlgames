...
from rlgames.agents.ac import load_ac_agent, ACAgent
from  rlgames.rl.experience import AdvExperienceCollector, adv_combine_experience
...
def load_agent(agent_path, args):
  if not os.path.isfile(agent_path):
    ...
    if args.model_size == 'small':
      from  rlgames.rl_models.ac_small import actor_critic_model
      model = actor_critic_model(encoder)
    else:
...
def setup_agent(agent_path, args):
  ...
  collector = AdvExperienceCollector()
...
def main():
  args = parse_args()
  temperature = 0.2
  agent_path = args.data_dir + '/agents/ac_' + args.model_size + '_' + args.encoder_name + '_' + str(args.board_size) + '.h5'
  ...
  for i in range(args.rounds):
    ...
    if round_no % 1000 == 0:
      ...
      exp = adv_combine_experience([collector1, collector2])
      ...
      agent2 = load_ac_agent(h5py.File(agent_path, 'r'))
