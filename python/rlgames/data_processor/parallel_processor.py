...
from rlgames.goboard import Board, GameState, Move
from rlgames.common_types import Player, Point
from rlgames.data_processor.index_processor import KGSIndex
from rlgames.data_processor.sampling import Sampler
from rlgames.data_processor.generator import DataGenerator
from rlgames.encoders import get_encoder_by_name

def worker(jobinfo):
  try:
    clazz, data_dir, encoder, zip_file, data_file_name, game_list = jobinfo
    clazz(data_directory=data_dir, encoder=encoder).process_zip(zip_file, data_file_name, game_list)
  except (KeyboardInterrupt, SystemExit):
    raise Exception('>>> Exiting child process.')

class DataProcessor:
  def __init__(self, data_directory, encoder='elevenplane'):
    self.encoder_string = encoder
    self.encoder = get_encoder_by_name(encoder, 19)
    self.data_dir = data_directory

  def load_data(self, data_type='train', num_samples=1000, use_generator=False, download=False):
  ...
  def map_to_workers(self, data_type, samples):
    ...
    for zip_name in zip_names:
      ...
      if not os.path.isfile(self.data_dir + '/' + data_file_name):
          zips_to_process.append((self.__class__, self.data_dir, self.encoder_string, zip_name,
