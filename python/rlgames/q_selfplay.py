...
from rlgames.agents.q import load_q_agent, QAgent
...
def load_agent(agent_path, args):
  if not os.path.isfile(agent_path):
    ...
    if args.model_size == 'small':
      from rlgames.rl_models.q_small import qlearning_model
      model = qlearning_model(encoder)
    else:
      raise ValueError('Unknown model size: {}'.format(args.model_size))
    agent = QAgent(model, encoder)
...
def main():
  args = parse_args()
  temperature = 0.3
  agent_path = args.data_dir + '/agents/q_' + args.model_size + '_' + args.encoder_name + '_' + str(args.board_size) + '.h5'
  ...
  for i in range(args.rounds):
    ...
    if round_no % 1000 == 0:
      ...
      collector2.clear()
  agent1.serialize(h5py.File(agent_path, 'w'))
  print('QLearing Selfplay complete. agent is in {}'.format(agent_path))
