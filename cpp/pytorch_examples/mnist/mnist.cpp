...
const char* kDataRoot = "/home/armandli/rlgames/data";
...
// object oriented way of creating a net, as compared to doing it the sequential way
...
void test(Net& model, torch::Device device, DataLoader& data_loader, size_t dataset_size) {
  ...
  for (const auto& batch : data_loader) {
    auto data = batch.data.to(device), targets = batch.target.to(device);
    auto output = model.forward(data);
    test_loss += torch::nll_loss(output, targets,/*weight=*/{}, Reduction::Sum).template item<float>();
